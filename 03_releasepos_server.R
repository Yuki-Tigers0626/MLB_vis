output$release_pos_xz_plot <- renderPlot({
  g <- ggplot() + 
    ...
    xlim(-max(abs(database()$release_pos_x))*1.1, max(abs(database()$release_pos_x))*1.1) + 
    ylim(0, max(abs(database()$release_pos_z))*1.1)
...
output$release_pos_xy_plot <- renderPlot({
  g <- ggplot() + 
    ...
    xlim(0, max(abs(database()$release_extension))*1.1) + 
    ylim(-max(abs(database()$release_pos_x))*1.1, max(abs(database()$release_pos_x))*1.1)
